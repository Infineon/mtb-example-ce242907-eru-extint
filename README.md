# PSOC&trade; Control C1 MCU: ERU extint

This code example demonstrates an event request unit (ERU) external interrupt (extint) in PSOC&trade; Control C1 MCU.
...
[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce242907-eru-extint)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[ERU extint](https://github.com/Infineon/mtb-example-ce242907-eru-extint)" application with the desired name "ERU extint" configured for the *KIT_PSC1M3_EVK* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSC1M3_EVK --app-id mtb-example-ce242907-eru-extint --user-app-name ERU extint --target-dir "C:/mtb_projects"
   ```
...
4. Confirm that the user LED toggles when the knob of the potentiometer (on P2.5) is turned clockwise and counter-clockwise
...
Port pin P2.5 is configured as an input port for the PSOC™ control C1 evaluation kit by the device configurator as follows:

**Figure 1. P2.5 Configuration**

![](images/psc1m3_port.png)


The PSOC™ control C1 evaluation kit uses ERU group0 with the event trigger logic on channel 1 (`ERU0_ETL1`). The output gating unit (`ERU_OGU`) is configured for service requests on the external trigger input.
...
Document title: *CE242907* – *PSOC&trade; Control C1 MCU: ERU extint*
